...
void top_peak_memory(
    ...
    #pragma HLS INTERFACE axis port=peak_in
    #pragma HLS INTERFACE axis port=peak_out_stream
    #pragma HLS INTERFACE ap_ctrl_none port=return

    //Hardcode period
    const ap_uint<32> INT_PERIOD = 20000; 
    ...
    if (!peak_in.empty()) {
        ...
        if (timer >= INT_PERIOD - 1) { 
            ...
            if (curr.val > p0.val) {
                max_peaks[0].val = curr.val; max_peaks[0].bin = curr.bin; max_peaks[0].count = 0;
                max_peaks[1] = p0;
                max_peaks[2] = p1;
                max_peaks[3] = p2;
            } else if (curr.val > p1.val) {
                max_peaks[1].val = curr.val; max_peaks[1].bin = curr.bin; max_peaks[1].count = 0;
                max_peaks[2] = p1;
                max_peaks[3] = p2;
            } else if (curr.val > p2.val) {
                max_peaks[2].val = curr.val; max_peaks[2].bin = curr.bin; max_peaks[2].count = 0;
                max_peaks[3] = p2;
            } else if (curr.val > p3.val) {
                max_peaks[3].val = curr.val; max_peaks[3].bin = curr.bin; max_peaks[3].count = 0;
            }
